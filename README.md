...
- 字幕写出前会按需拆分，单条字幕最长不超过 30 字，并根据拆分结果等比分配时间戳
...
为了避免单行字幕过长，程序在写入 SRT 之前会调用 `wrap_srt_text`：

1. 先根据句号、问号、感叹号等标点划分成句子；
2. 若句子仍超过 30 个字符，进一步在逗号、空格等处寻找合适的切分点；
3. 如果缺少任何分割标记，则在第 30 个字符处硬性切分；
4. 一段文本被拆成多行时，`write_srt` 会按行数等比分配原始时间戳，输出为多条字幕。

这样的策略既保持句子尽量完整，又确保不会丢失任何文字。
